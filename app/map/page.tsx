...
export default function SpatialHub() {
  ...
  return (
    <div ref={container} className="space-y-6 max-w-4xl mx-auto pb-10">
      <div className="anim-item flex flex-col md:flex-row justify-between items-start md:items-end gap-4 mb-8 border-b border-border-subtle pb-6">
        <div>
          <h2 className="text-4xl font-heading font-black text-text tracking-tight uppercase">Spatial Hub</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {rooms.map((room) => (
          <div key={room.id} className="anim-item bg-card rounded-card border border-border-subtle p-6 hover:shadow-[0_0_20px_var(--primary)] transition-all cursor-pointer group relative overflow-hidden">
             ...
             <div className="flex justify-between items-start mb-4 relative z-10">
               <div>
                 ...
                 <h3 className="text-xl font-heading font-bold text-text leading-tight">{room.name}</h3>
               </div>
             ...
             <div className="relative z-10 pt-4 border-t border-border-subtle flex justify-between items-center">
               <div className="flex -space-x-2">
                 {[...Array(Math.min(room.occupancy, 4))].map((_, i) => (
                   <div key={`${room.id}-${i}`} className={`w-8 h-8 rounded-full bg-background border-2 border-card flex items-center justify-center text-[10px] font-bold border-${room.theme}/30 text-text`}>
